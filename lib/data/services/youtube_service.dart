...
///
/// Para Web (PWA), todas las llamadas externas usan el proxy 'allorigins' para evitar CORS.
...
class YouTubeService {
  ...
  // Proxy CORS principal para Web
  static String _proxied(String url) =>
      'https://api.allorigins.win/raw?url=${Uri.encodeComponent(url)}';

  // Proxy CORS secundario (fallback)
  static String _backupProxy(String url) =>
  ...
  // ─────────────────────────────────────────────────────────────
  // METODO 1: Scraping YouTube vía proxy (Web) o directo (Móvil)
  // ─────────────────────────────────────────────────────────────
  ...
  Future<String?> _searchYoutubeScrape(String query) async {
    ...
    final urls = kIsWeb
        ? [_proxied(ytSearchUrl), _backupProxy(ytSearchUrl)]
        : [ytSearchUrl];
  ...
  Future<String?> _searchPiped(String query) async {
    for (final filter in ['music_songs', 'videos']) {
      for (final base in _pipedInstances) {
        ...
        final urls = kIsWeb
            ? [_proxied(pipedUrl), _backupProxy(pipedUrl)]
  ...
  Future<String?> _searchInvidious(String query) async {
    for (final base in _invidiousInstances) {
      ...
      final urls = kIsWeb
          ? [_proxied(invUrl), _backupProxy(invUrl)]
          : [invUrl];
